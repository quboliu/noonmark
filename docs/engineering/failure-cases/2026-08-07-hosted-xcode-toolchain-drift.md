...
- 待完成：同一 run 后来因独立的 `FAIL-2026-08-07-06` 缺少 `rg` 而中断；build 14 已证明 hosted 工具安装后又因独立的 `FAIL-2026-08-07-07` 测试同步竞态停止；build 15 必须完成剩余 hosted check、真实 App E2E 与 canonical DMG。
...
build 12 已因本故障与独立 workflow 故障退役；build 13 证明 Xcode 根因修复后，因独立的工具依赖故障退役；build 14 又因独立的测试同步竞态退役。build 15 必须完成 Xcode 26.2 普通 CI 再建立 tag。若 GitHub image 不再提供固定路径，workflow 必须 fail-closed 并退役该 build，再由人工评估新的项目工具链；不得静默回到 image 默认值、删除产品能力或跳过 hosted build。
