...
产品现已取消页面 Composer 的「收起／展开」交互。这个历史故障所涉的用户意图不再属于产品能力，因此不能继续保留收起状态机、紧凑输入样式或相关按钮。页面输入框固定为完整编辑面；草稿自动保存、Markdown 工具、候选分组／标签、发布与原位编辑不变。
...
- fast：`scripts/test-notes-ui-contract`，由 `scripts/check` 强制调用，禁止恢复飞光 Composer 的紧凑样式、收起状态或「收起／展开」文案，并要求真实 App 断言存在。
- symptom：`scripts/test-e2e`，由 `scripts/test-all` 强制调用，在真实 App 打开飞光时检查输入框初始就是完整编辑面，且不存在第二个收起／展开操作。

## 发行与回滚

只使用 `e2e` 与 `demo` profile。若固定展开造成草稿丢失或发布，停止交付并回退本次状态边界；不得借由恢复收起状态或清空草稿来规避问题。

## 教训与永久约束

派生 UI 状态不能吞掉同名用户 intent；当产品取消一个交互时，必须同时移除其状态机、文案、样式和旧症状门禁，并改用真实 App 门禁证明该入口不存在。
