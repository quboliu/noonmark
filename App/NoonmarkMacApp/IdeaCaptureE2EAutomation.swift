...
struct IdeaCaptureE2EAutomation: LaunchAutomationRunnable {
    ...
    private func exercise(on store: NoonmarkStore) async throws {
        ...
        try await exerciseUnresolvedClassificationDiagnostic(
            store: store,
            mainWindow: mainWindow,
            input: input
        )
    ...
    private func exerciseUnresolvedClassificationDiagnostic(
        store: NoonmarkStore,
        mainWindow: NSWindow,
        input: WindowServerInputDriver
    ) async throws {
        guard let editor = AppViewTreeE2E.view(
            ...
            throw Failure.failed(
                "Flylight unresolved classification lost its editor"
            )
        }
        try await click("ideas.composer.input", in: mainWindow, input: input)
        try await waitUntil(
            "Flylight unresolved classification did not receive focus"
        ) {
            editor.window?.firstResponder === editor
        }
        try input.typeUnicode(Self.unresolvedClassificationDraft)
        try await waitUntil(
            "Flylight unresolved classification draft did not reach the editor"
        ) {
            editor.string == Self.unresolvedClassificationDraft
                && store.ideaText == Self.unresolvedClassificationDraft
        }
        try input.postKey(keyCode: 36, modifiers: [.command])
        try await waitUntil(
            "Flylight unresolved classification did not preserve its draft"
        ) {
            store.ideaComposerSession.submissionState == .failed
                && store.ideaComposerSession.failureMessage != nil
                && editor.string == Self.unresolvedClassificationDraft
                && store.engine.ideaTimeline().contains(where: {
                    $0.body == Self.unresolvedClassificationDraft
                }) == false
        }
        try await assertUnresolvedClassificationWasRecorded(store: store)
        try input.postKey(keyCode: 0, modifiers: [.command])
        try input.postKey(keyCode: 51)
        try await waitUntil(
            "Flylight unresolved classification draft did not clear"
        ) {
    ...
    private func assertUnresolvedClassificationWasRecorded(
        store: NoonmarkStore
    ) async throws {
        guard let recorder = store.localDiagnosticRecorder else {
            throw Failure.failed(
                "Flylight unresolved classification lost its diagnostic recorder"
            )
        }
        for _ in 0 ..< 120 {
            let package = try await recorder.snapshotPackage()
            if package.records.contains(where: { record in
                record.event.code == .mutationRejected
                    && record.event.mutationContext == .idea
                    && record.event.mutationRejectionReason == .domainRule
        ...
        throw Failure.failed(
            "Flylight unresolved classification was not durably recorded"
        )
    }
