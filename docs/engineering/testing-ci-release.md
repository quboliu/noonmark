...
- 腾讯输入法专项：`make test-tencent-ime-input-contract` 是可在 hosted runner 执行的快速门禁，只校验共享 catalog、真实输入源与生命周期能力等稳定语义。`make test-tencent-ime-release-smoke`、`make test-tencent-ime-input-matrix` 和 `make test-tencent-ime-termination-persistence` 依赖真实腾讯拼音、稳定签名、Input Monitoring 和 WindowServer，因此只作为 push 前本地全集自测，不进入 GitHub workflow，也不被 tag publisher 消费。发行 smoke 在腾讯输入源未安装或未启用时会产出可审计的 `SKIPPED` 证据并继续；输入源可用时仍阻断组合态、即时退出或 SQLite 重启回读的失败。完整方法与输入面表见 `docs/engineering/tencent-ime-input-performance.md`。
...
- Live iCloud Sync：真实 Apple Account / iCloud Drive 手动测试，入口为 `NOONMARK_LIVE_ICLOUD_PACKAGE_PATH=/absolute/current-package.json scripts/test-icloud-sync-live`，不进入默认 `make check`；数据包路径为必填且缺失时 fail-closed，固定运行 `e2e` profile，只使用 `Noonmark-E2E/LiveTests/<UUID>`，覆盖真实数据包双 SQLite 往返、record merge、真实 `.app` 同步、SQLite status、per-producer head／immutable batch 与 `brctl` 上传完成信号。它不得读取或写入 production `Noonmark/SyncRepository`。
...
- `CFBundleShortVersionString`／`CFBundleVersion`：唯一来源是受 Git 管理的 `release/VERSION`，当前为 `0.2.1`／`6`。构建拒绝版本环境覆写；精确 tag、build 递增与兼容矩阵见 `docs/engineering/versioning-and-private-release.md`。
...
make package-dmg # 委托完整 scripts/release-private-dmg 发行链
make verify-dmg
...
- 腾讯输入 53 面性能矩阵与 24 面退出／重启矩阵保留为本地专项入口，不由 GitHub 调度。
- 保留测试输出，方便复现 seed。

私有 Apple Development DMG 发行验收：

- 本地先跑通完整自测全集，但本地 evidence 不是线上发布依赖。
- 正式发版由 tag 触发的 `.github/workflows/release-publish.yml` 承担：它验证该精确 commit 的单一 GitHub-hosted `ci.yml` job 已成功，不重跑 `scripts/check`；随后执行 `scripts/package-dmg release` 与 `scripts/verify-dmg`，创建公开 GitHub Release 并上传 DMG 与 SHA-256（见 `.github/RELEASING.md`）。
...
- GitHub Release 只接受 tag workflow 新建，不得覆盖既有 Release 或 asset；分发正本为 CI 签名与静态验证后上传的 DMG。
...
- 2026-07-14：`scripts/test-icloud-sync-live` 已通过当时的旧 full-mirror 仓库证据。ADR-0046 cutover 后，live 门禁改为覆盖双 SQLite 合并、真实 E2E App 显式启用与同步、`localFirst.sync.lastStatus` 落盘，以及 immutable `batches/`／per-producer `heads/` 的 CloudDocs 上传确认。它仍不是两台物理设备或 CloudKit production 验收。
