...
struct DatePickerSheet: View {
    ...
    private var formatter: AppDateTimeFormatter {
        AppDateTimeFormatter(
            ...
            timeZone: TimeZone(identifier: store.naturalDayState.timeZoneIdentifier)
                ?? .autoupdatingCurrent
        )
    }
    ...
    private var firstAllowedDate: Date? {
        guard state.purpose.allowsPastDates == false else { return nil }
        let firstDate = state.purpose.futureOnly
            ? NoonmarkStore.offset(store.today, by: 1)
            : store.today
        return try? formatter.foundationDate(from: firstDate)
    }

    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text(state.purpose.title(copy: store.copy))
                .font(.noonmarkSystem(size: 17, weight: .semibold))
                .accessibilityAddTraits(.isHeader)
            Text(state.purpose.rangeHint(copy: store.copy))
                .font(.noonmarkSystem(size: 11.5))
                .foregroundStyle(Theme.text3)
                .fixedSize(horizontal: false, vertical: true)

            Group {
                if let firstAllowedDate {
                    DatePicker(
                        "",
            ...
            HStack {
                Spacer()
                Button(store.copy.cancel) { store.showingPicker = nil }
                    .keyboardShortcut(.cancelAction)
                Button(store.copy.chooseDate) {
                    confirmSelection()
                }
        ...
        .onAppear {
            ...
            didInitialize = true
            do {
                pickedDate = try formatter.foundationDate(from: initialLocalDate)
            } catch {
                validationMessage = store.copy.invalidDateSelection
            }
        }
    }
    ...
    private func confirmSelection() {
        do {
            let localDate = try formatter.localDate(from: pickedDate)
            if state.purpose.futureOnly, localDate <= store.today {
                validationMessage = state.purpose.rangeHint(copy: store.copy)
                return
            }
            if state.purpose.allowsPastDates == false, localDate < store.today {
                validationMessage = state.purpose.rangeHint(copy: store.copy)
                return
            }
            validationMessage = nil
            store.performDateChoice(localDate)
        } catch {
            validationMessage = store.copy.invalidDateSelection
        }
    }
