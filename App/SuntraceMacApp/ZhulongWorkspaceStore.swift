...
final class ZhulongWorkspaceStore: ObservableObject {
    ...
    func applyCurrentTodoDiff(
        to currentEngine: SuntraceEngine,
        today: LocalDate,
        persistEngine: (SuntraceEngine) throws -> Void,
        now: Date = Date()
    ) -> SuntraceEngine? {
        guard let session = selectedSession else { return nil }
        do {
            var afterSession = session
            var afterEngine = try SuntraceEngine(snapshot: currentEngine.snapshot())
    ...
    func recoverPendingApplication(
        ...
        do {
            ...
            let recoveredEngine: SuntraceEngine
            if currentDigest == beforeDigest {
                recoveredEngine = try SuntraceEngine(snapshot: pending.afterSnapshot)
