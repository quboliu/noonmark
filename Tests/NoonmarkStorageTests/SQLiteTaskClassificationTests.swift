...
final class SQLiteTaskClassificationTests: XCTestCase {
    ...
    private struct ConcurrentRemovalFixture {
        let laterSnapshot: NoonmarkSnapshot
        let mergedSnapshot: NoonmarkSnapshot
    ...
    private func makeConcurrentRemovalFixture() throws -> ConcurrentRemovalFixture {
        ...
        let laterEngine = try NoonmarkEngine(snapshot: baseSnapshot)
        ...
        let earlierEngine = try NoonmarkEngine(snapshot: baseSnapshot)
    ...
    private func makeDatabaseURL(_ stem: String) -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("noonmark-\(stem)-\(UUID().uuidString)")
