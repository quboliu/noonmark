...
- 2026-07-06：详情态视觉回归已纳入默认门禁，当前基线为：day-detail `changed_ratio=0.151978`，day-manual-detail `0.151978`，day-review-saved `0.136030`，pool-detail `0.093695`，future-detail `0.065242`，unfinished-detail `0.144671`，completed-detail `0.151434`。未完成详情和已完成父任务详情均已收敛为原型的通用任务详情栏结构，不再额外显示专用处理区、统计卡、锁定提示、子任务快照或底部重复操作区。详情态阈值先以防回归为主，后续 UI 复原会逐页收紧。
