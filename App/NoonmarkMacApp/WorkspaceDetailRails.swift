...
struct DetailRail: View {
    ...
    private func routeContent(_ route: NoonmarkStore.DetailRailRoute) -> some View {
        ...
        case .calendar:
            EmptyView()
        case .flylight:
            // The timeline already owns a Flylight's body, timestamp,
            // classification, and per-item actions. Do not repeat those facts
            // in the shared rail just because it is available.
            EmptyView()
        case .selection:
            selectionDetail
        case .zhulong:
