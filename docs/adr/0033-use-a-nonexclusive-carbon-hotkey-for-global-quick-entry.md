...
- 保存前拦截晷迹自身命令和 `CopySymbolicHotKeys` 返回的已启用系统组合；第三方 App 冲突只能明确说明并由用户在常用 App 中试按。
...
- `NoonmarkMacRuntimeTests` 覆盖默认值、显示文本、损坏偏好 fail-closed、硬冲突、禁用和注册失败保留旧组合。
- 真实 `.app` E2E 通过设置页录制新组合，证明旧注册已移除；随后关闭主窗口、激活 Finder、由 WindowServer 发送新组合、连续触发、输入和 Return，并验证任务唯一落库、主窗口不重开及 Finder 焦点恢复。
