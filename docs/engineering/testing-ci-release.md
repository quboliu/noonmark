...
- 2026-07-06：当前多页面视觉指标为：day `changed_ratio=0.131998`，pool `0.086481`，future `0.096563`，unfinished `0.114055`，completed `0.111986`，calendar `0.075295`，settings `0.238790`；settings 页已从旧 HTML 原型的最小设置骨架升级为包含烛龙 Provider 状态、数据包指标和隐私边界的实现基线，阈值为 `0.25`。后续视觉收紧以真实可用性、信息层级和明显偏差为准，不追求演示数据条数逐项一致。
