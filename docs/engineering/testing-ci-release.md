...
- 2026-07-06：当前多页面视觉指标为：day `changed_ratio=0.135071`，pool `0.052542`，future `0.041098`，unfinished `0.098300`，completed `0.129391`，calendar `0.076122`，settings `0.052011`；settings 页已回到 HTML 原型的单栏信息架构，分段控件已回到原型紧凑宽度，同时把烛龙 Provider 保留为同步区后的紧凑折叠入口，阈值收紧为 `0.08`；pool 页列表已回到原型单行密度，任务行首已恢复为原型的 18px 灰色虚线空圆占位符，阈值收紧为 `0.07`；future 页列表已移除首屏计数和行内操作噪声，日期组跳转链接已回到原型标题旁，并通过共享排序控件外壳收敛把阈值收紧为 `0.05`；day 页行内轨迹元信息已恢复“延续次数 + 持续天数”的原型语义，新增复盘区烛龙分析入口后阈值保持 `0.15`；unfinished 页行密度、状态图标、元信息纯文本分隔和已延续到今天文案已向原型收敛，阈值收紧为 `0.10`；calendar 右栏当天 badge、任务数量文案和选中日任务轨迹元信息已按原型收敛，阈值收紧为 `0.09`；completed 页移除额外计数、主任务完成标签和子任务完成时间，并以稳定行高保持原型卡片节奏，阈值收紧为 `0.14`。后续视觉收紧以真实可用性、信息层级和明显偏差为准，不追求演示数据条数逐项一致。
