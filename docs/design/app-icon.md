...
晷迹只保留一个正式 App 图标身份：**晷迹 01 macOS 图标版（Noonmark 01 macOS Icon）**。它以仓库内的 SVG 为唯一正式视觉源，保留原有日晷语义，同时按 macOS Dock 的实际显示比例重构主体占比和笔画重量。
...
- 图标内容：完整保留白色背景、黑色日晷主体、黑色晷针、橙色日面与橙色刻度。
- 主符号占画布约 `60% × 62%`，有效符号像素约占画布 `10.6%`；同机 256 px TickTick 图标的取证基准约为 `55.5% × 55.9%` 和 `10.9%`。该基准只用于校准 Dock 视觉量级，不复制其勾选语义或造型。
- 黑色主弧笔画为设计画布的 `54／1024`，橙色弧与刻度为 `38／1024`；较旧 raster 版本不再作为兼容基线。
