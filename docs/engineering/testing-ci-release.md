...
- 2026-07-06：详情态视觉回归已纳入默认门禁，当前基线为：day-detail `changed_ratio=0.151100`，day-manual-detail `0.151100`，day-review-saved `0.136030`，pool-detail `0.093695`，future-detail `0.065242`，unfinished-detail `0.143756`，completed-detail `0.152561`。Day Todo 详情已使用中文日期、单行轨迹上下文卡和“任务轨迹”章节标题；手动完成进度已从系统 `Slider` 改为确定性自绘滑杆，恢复蓝色已完成轨道、可见 thumb、5% 刻度和可访问调节入口；任务轨迹当前节点已补齐 accent-soft 背景；未完成详情和已完成父任务详情均已收敛为原型的通用任务详情栏结构；完成态详情已用自绘进度条恢复 100% 绿色，并把完成池选中态从状态绿改回通用蓝色选中态。详情态阈值先以防回归为主，后续 UI 复原会逐页收紧。
