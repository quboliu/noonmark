...
- 2026-08-04：K3 最终 review 指出上述 fixture 只是未解析分类，不能证明真实 persistence failure 的恢复能力；门禁升级为真实 SQLite writer contention，解锁后同一草稿物理重试并成功写入。

## 复现与证据

运行 `NOONMARK_E2E_IDEA_CAPTURE_ONLY=1 scripts/test-e2e`。最初修复前，视图树同时显示 `ideas.composer.surface=failed`、错误说明与 `ideas.composer.primary=重试`，但 `buttonInteractionTarget` 无法解析真实点击区域，门禁返回 `failed publish did not expose an actionable retry`。

当前门禁直接在隔离 E2E 数据库执行 `BEGIN IMMEDIATE`，第一次 `⌘Return` 必须进入 failed，同时逐字保留原生 editor、session、first responder 与 selected range，Engine 和磁盘均零写入。释放 writer lock 后再以 WindowServer 点击「重试」，同一草稿成功写入、重启并由 SQLite 回读。

## 排除的假设

- 不是按钮文案未刷新：失败现场视图树已经显示「重试」。
- 不是草稿被清空：session 与原生编辑器均保留完整失败草稿。
- 不是失败提交产生部分写入：持锁期间引擎时间线与另一个 `SQLiteEngineRepository.load()` 均确认目标正文不存在。
...
- 真实 App E2E 不只读标题，而是制造真实 SQLite writer failure、解析命中区、WindowServer 物理点击，并验证失败前后草稿／焦点／选区和最终持久化。

## 验证结果

- `NOONMARK_E2E_IDEA_CAPTURE_ONLY=1 scripts/test-e2e`：真实 SQLite contention、失败态物理重试、草稿／焦点／选区保留、零误写及解锁后成功通过。
- `scripts/test-notes-ui-contract`：通过。
- `make check`：通过；完整工作树的 build、UT、IT、ST、确定性仿真、契约、SwiftLint 与 SwiftFormat 门禁均为绿。

## 永久门禁

- fast：`scripts/test-notes-ui-contract`，由 `scripts/check` 强制调用；约束 failed 非空草稿可重试，并要求真实 `BEGIN IMMEDIATE`、选区保持与物理点击路径不得降级。
- symptom：`scripts/test-e2e`，由 `scripts/test-all` 强制调用；真实 App 在 SQLite writer contention 下失败，显示说明、保留草稿／焦点／选区与零误写，解锁后物理点击重试并成功回读。
