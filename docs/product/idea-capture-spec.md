...
- 页面空 composer 固定为完整编辑面，高度基线 112pt；不提供「收起／展开」状态、按钮或紧凑样式。placeholder 与 caret 使用同一个原生 `NSTextView` 坐标空间。
- `#标签`／`@分组` 分类 scanner 不得误读 Markdown heading、代码、转义字面量、显式链接目标或裸 URL。
- 双击旧条目正文原位编辑；三点菜单只提供辅助编辑入口。

### 浏览

- 最近集合按创建时间倒序、按自然日分组；所有活动飞光都留在时间线，包括已经加入 Sticky Note 的条目。
- 飞光沿用工作区共享右栏外壳与全局快速检索；右栏内容区不渲染独立详情、空状态、选中条目正文、时间、分类或逐条操作，保持留白。用户仍可用页头或栏边界动作收起／展开，收起后时间线占满主内容区，窗口恢复沿用统一工作区状态。
- 飞光卡片使用块级 Markdown renderer；右栏不得再次渲染正文。
- 搜索、分组／标签过滤与回看继续是互斥主集合，不堆叠多段列表。

### 操作

- 飞光里的活动条目提供编辑、删除、加入／移出 Sticky Note。
...
- 页面 slogan 使用「把值得反复看见的内容，留在手边。」；所有界面文案只把「飞光／Flylight」作为模块入口名称，单项必须称为记录、内容或条目，不得把模块名量词化、复数化或当成单条 note 使用。
...
- fast gate：侧边栏三分组顺序、飞光语言、共享可收起右栏外壳、默认完整 composer、右栏不重复正文、Sticky Note 单一下拉入口、两种布局 seam、隐藏墓碑与 Markdown contract。
- domain／storage：精选加入／移出、源条目继续出现在飞光、编辑同步、删除清除精选、SQLite round-trip 与 sync 收敛。
- symptom gate：真实 `.app` 验证飞光 composer 初始为完整编辑面且没有收起／展开第二操作，右栏不含选中条目正文；再物理收起／展开右栏，并点击 Sticky Note 下拉清单切换两种展示视图；返回飞光仍可见，重启后关系与视图正确。
