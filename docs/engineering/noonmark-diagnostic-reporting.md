...
晷迹 `0.1.1 (2)` 起使用有界的本机诊断证据。App 不会自动上传诊断资料；用户必须先查看摘要，再自行选择保存位置。App 管理的诊断文件实际占用不超过 4 MiB、最长保留 7 天，用户主动保存的单个 `.noonmarkdiagnostics` 导出包不超过 8 MiB，因此问题发生后应尽快导出。
...
3. 选择“导出诊断资料…”，保存不超过 8 MiB 的 `.noonmarkdiagnostics` 文件。导出过程不自动发送，也不读取 Todo 正文、同步 payload、Provider 内容或 Keychain 凭证。
...
首次报告的同步停滞、修改被拒绝与重启后同步未完成，目前没有足够现场证据可以认定根因。自动化中的 lock wait、写入失败或 SIGKILL 是采集能力探针，不是对真实事故原因的替代解释；开发者只有在用户提交下一次真实诊断包后，才可按证据重新提出和排除假设。
...
开发与测试不得为了取证直接启动 production App，或读取、定位、探测、reset 用户的 production 数据根、iCloud repository 与 Keychain。开发者只分析用户主动提供的诊断包、版本信息和可选 `.ips`；删除同步目录、导入 canonical JSON 及安装私有 Apple Development DMG 都由用户本人明确执行。
