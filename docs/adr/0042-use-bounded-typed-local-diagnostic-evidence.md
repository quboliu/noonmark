...
本 ADR 解决“下一次故障是否能留下可验证现场”，不宣称已经复现、定位或修复首次宿主机同步停滞与修改拒绝的根因。故障注入只验证采集链，不能替代真实事故证据。
...
- 写入必须在 append 前先轮换，并把 `.tmp` 与原子替换过程纳入 4 MiB 配额；受控文件的 allocated size 无法证明时 fail-closed 停止文件写入，只保留系统日志。诊断根与 Metric cache 目录必须在 recorder 初始化时建立并以不跟随符号链接的方式打开，由进程在整个 recorder 生命周期持有各自 descriptor；受控项目的枚举、读取、建立、替换和清除只允许相对该 descriptor 使用 `openat`、`fstatat`、`renameat` 和 `unlinkat`，目录路径在运行中被换成 symlink 或另一真实目录时仍只操作原先持有的 inode。受控 regular file 必须先用 `fstatat` 捕获 identity，再用 `openat` 打开并以 `fstat` 同时对账当前 euid、单一 hard link、device 与 inode；allocated size、mtime 和 bounded data 必须全部从这同一个 nonblocking descriptor 取得，不能验证一次后按名字再次打开。symlink、FIFO、hardlink 或其他非预期 entry 只能移除诊断目录内的名字，不得读取、删除或改写其外部目标。用户主动导出的 staging 位于持久诊断根之外，输出包不超过 8 MiB，并在成功或失败后清理 staging。
...
- 诊断写入由专用有界队列处理，不在 MainActor 执行文件 I/O，也不取得 Todo、同步或导入锁。队列最多容纳 256 条或 256 KiB；溢出、segment 轮换、active marker／事故胶囊压缩和 Metric cache 淘汰都必须留下 typed loss counter。loss counter 与 allocated-size 峰值使用最多八个 UTC 日桶，按当前七日覆盖窗口聚合，不得在旧窗口到期时整体清零而抹掉较新的 partial 证据；若 cutoff 落在某 UTC 日中间，则该边界日整桶丢弃，未来或畸形日桶也必须丢弃，确保绝不导出超过七日的证据。持久 sequence 必须大于零且为下一次分配保留空间；损坏 segment 或 index 造成 sequence 空间耗尽时必须排除损坏记录或关闭 file sink，并以 checked addition 计算下一序号，不能触发不可捕获的整数 trap。诊断系统任何权限、磁盘、编码或损坏错误都 fail-open，不得改变用户操作或同步的成功／失败语义。
...
- release 必须保留 `.app`、dSYM、Mach-O UUID 与 binary SHA 的对应关系；没有匹配 dSYM 的构建不得宣称能可靠分析用户提供的 `.ips`。production DMG 只做静态身份门禁，真实交互与 DiagnosticReports 门禁只运行从该 package 受控派生的 `dmg-validation` App，且必须证明 `production_app_executed=false`。若用户日常使用中产生真实 `.ips`，必须先与候选 manifest／dSYM 对账再符号化，不得用 fixture 或伪报告替代。MetricKit 是补充渠道，不得作为业务 operation 是否完成的唯一证据，也不得使用不安全的 signal handler 补写日志。
...
- 故障注入闭环只能证明相应 operation／incident、阶段、拒绝与跨重启证据能被采集；在真实用户资料到达前，最初同步故障的根因仍为未知，不得把注入条件外推成结论。

## Risk And Rollout

- 风险等级：P。主要风险是敏感正文泄漏、日志风暴突破配额、诊断 I/O 阻塞用户操作、半写 segment 破坏历史、跨重启错误归因，以及错误清除业务数据。
- 灰度：先以纯契约验证 typed schema、容量和隐私，再在隔离 `e2e` App 闭合真实 UI 导出与跨重启证据，随后对 production DMG 做纯静态身份门禁，并以受控派生的 `dmg-validation` App 完成动态验收；最后才把稳定 Apple Development 签名 DMG 私下交给用户自行下载和安装。任何阶段都不启动开发／测试 production runtime、不启用远程上传，也不公开分发。
...
- 公共 seam 测试覆盖一百万事件风暴、并发 producer、单条和 MetricKit 超限、append 前轮换、`.tmp` 配额、allocated-size 上限、队列过载、损坏／半写 segment 恢复、`UInt64.max` sequence／nextSequence 子进程探针、权限／磁盘失败 fail-open、schema 兼容与导出 digest；目录边界测试在 recorder 运行中替换 root 与 Metric cache，在文件 identity capture 后替换同名 inode，并逐阶段对账外部 canary 的内容、device、inode、mtime 和 size；symlink、hardlink 与 FIFO 也必须证明 snapshot、export 和 clear 都不越界。
...
- production DMG 验收必须静态对账 checksum、签名、版本身份、Mach-O UUID、dSYM、最终 binary SHA 与 source-linked SHA，并固定证明 `production_app_executed=false`；受控派生的 `dmg-validation` App 另以真实 WindowServer、SQLite 写入、退出／重启、unified log 与 DiagnosticReports 差分完成动态门禁。该门禁不宣称 production bundle 已运行，也不伪造 `.ips`。
