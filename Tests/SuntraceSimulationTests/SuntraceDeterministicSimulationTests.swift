...
final class SuntraceDeterministicSimulationTests: XCTestCase {
    ...
    private func assertInvariants(_ engine: SuntraceEngine, today: LocalDate) throws {
    ...
    private func choosePendingTrace(_ engine: SuntraceEngine, rng: inout SeededGenerator) -> DayTrace? {
        chooseTrace(engine.traces.values.filter { $0.status == .pending }, engine: engine, rng: &rng)
    }

    private func chooseCurrentPendingTrace(_ engine: SuntraceEngine, today: LocalDate, rng: inout SeededGenerator) -> DayTrace? {
        chooseTrace(engine.traces.values.filter { $0.status == .pending && $0.date == today }, engine: engine, rng: &rng)
    }

    private func chooseFuturePendingTrace(_ engine: SuntraceEngine, today: LocalDate, rng: inout SeededGenerator) -> DayTrace? {
        chooseTrace(engine.traces.values.filter { $0.status == .pending && $0.date > today }, engine: engine, rng: &rng)
    }

    private func chooseCurrentPendingTraceWithoutSubtasks(_ engine: SuntraceEngine, today: LocalDate, rng: inout SeededGenerator) -> DayTrace? {
    ...
    private func chooseCurrentPendingSubtask(_ engine: SuntraceEngine, today: LocalDate, rng: inout SeededGenerator) -> Subtask? {
    ...
    private func chooseTrace(_ values: [DayTrace], engine: SuntraceEngine, rng: inout SeededGenerator) -> DayTrace? {
    ...
    private func chooseSubtask(_ values: [Subtask], engine: SuntraceEngine, rng: inout SeededGenerator) -> Subtask? {
        choose(values.sorted { lhs, rhs in
