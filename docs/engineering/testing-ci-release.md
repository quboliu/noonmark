...
- 2026-07-06：当前多页面视觉指标为：day `changed_ratio=0.131998`，pool `0.086481`，future `0.096563`，unfinished `0.114055`，completed `0.111986`，calendar `0.075295`，settings `0.061362`；settings 页已回到 HTML 原型的单栏信息架构，同时把烛龙 Provider 保留为同步区后的紧凑折叠入口，阈值收紧为 `0.12`。后续视觉收紧以真实可用性、信息层级和明显偏差为准，不追求演示数据条数逐项一致。
