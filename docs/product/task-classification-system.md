...
- 显式分类的任务创建仍与分类原子执行；普通新任务则与自动归类工作原子保存并立即可用，随后分类项、关系、审计、同步 outbox 与工作完成状态另行原子发布。
...
- 不提供烛龙对旧任务、批量草稿或普通建议的后台无人确认写入；只允许 ADR 0026 定义的新任务自动归类在严格 fence 下复用或建立分类项。
...
- 来源至少区分用户直接操作、分类继承、烛龙建议后用户确认、新任务自动 AI 归类和确定性领域动作。同步只是运输方式，不得替换原来源。
...
直接用户操作与烛龙批量操作共用 Interface。烛龙批量路径只能产生 intent 和计划，不能构造有效 confirmation；用户确认后的 AI 关系与用户直接关系拥有相同保护，来源不产生未来修改权限。新任务自动归类另用 Module 签发的短效、单任务 authority，只能在任务内容、分类基线、目录 revision 和工作 generation 均未变化时提交。

## 新任务自动归类

- 用户可以先建立分组与标签，也可以保持空目录；每条从 Day Todo、未来日期、任务池、全局快速输入或已确认 Todo diff 新建的任务都会建立本机持久工作。
- 创建事务不调用网络。Provider 未配置、离线或失败时任务照常存在，并显示等待／失败与重试状态。Provider 不可用期间形成的历史积压必须由用户明确选择开始、跳过或稍后处理；保存配置本身不是批量付费授权。
- 请求只使用标题、描述和 active 分类目录；成功结果恰好包含一个分组与一至三个 AI 标签，优先复用现有项，没有合适项才新建。
- 快速输入的显式 `#标签` 与 `@分组` 属于用户事实，自动路径只能保留或补充，不能覆盖用户主分类；等待期间任何用户分类变化都会使旧 proposal 作废。若仅任务文字或目录发生变化，则旧工作结束，并以新 generation 重新取得 context 后自动归类。
- 付费 dispatch authorization 与 Provider 健康 circuit 独立持久化。已授权工作可在重启或 Provider 恢复后继续；未授权历史工作永不因启动、保存相同配置或测试连接而自动放行。401／403 全局阻塞，429、5xx、超时和断网使用全局冷却及单一 half-open probe，不能按积压数量逐项烧穿失败请求。
- Provider 请求成功后先保存严格 typed proposal；只要 checkpoint 已落盘，Provider circuit、App 重启或本地提交重试都不得再次调用 LLM。`proposalReady` 只执行本地 fence 验证与原子提交。
- 导入、同步、seed、延续、回池、变更链和已有分类继承不入队。撤销新任务会取消工作并移除仅由该工作建立的当前关系，重做只恢复本次撤销取消的工作。
- 队列是本机运行基础设施，不进入 `NoonmarkSnapshot`、同步或数据包；分类成功后的普通事实继续同步。

## 快速捕获事务

Day Todo、未来日期、任务池与全局快速记录共用一套输入语法：`#名称` 表示标签，`@名称` 表示唯一主分类。符号只在词边界触发，不能误判邮箱中的 `@` 或 `C#`；名称含空格时使用 `#"Deep Focus"` 或 `@"Client Work"`。输入第二个 `@分组` 必须阻止提交并明确提示，不能静默采用第一个或最后一个。

输入 `#` 或 `@` 时展示对应 active 目录候选；选择含空格候选时必须自动写成可无损解析的引号形式。快速记录弹窗与页面内快速新增必须使用同一解析器、候选排序和完成规则。

快速捕获必须先完整解析并 prepare，再一次提交：任务链、任务定义、目标 Day Todo／任务池、显式新分组、所有新标签、主分类关系、标签关系、分类变更记录和 sync outbox。重复 `#标签` 可以去重，但 receipt 必须披露；超过三个标签必须全部保存；任何 blocker 或持久化失败都不得留下半成品任务。
...
> 在晷迹当前 SwiftUI Mac App 的亮色工作台上实现“分组与标签”，使用用户确认的当前紧凑密度，严格保持 1200×768 默认窗口、960×720 最小窗口、约 220pt 左侧导航、主内容区和约 280pt 右侧详情 rail；右侧 rail 默认收起，切换时窗口尺寸不变，由主内容区自适应让出或收回宽度。背景使用现有 `Theme.panel/panel2`，发丝线使用 `Theme.line`，圆角维持 7–8pt，不新增暗色大面板、重阴影或孤立 Dashboard。任务行先显示一个不带 `#` 的分组 badge：高度 20pt、水平 padding 8pt、6pt 色点、10.5pt semibold、1px 稍强描边；其后显示最多两个带 `#` 的对等标签 chip，再显示同高 `+N` chip，点击以 popover 展开全部，绝不截断数据。快速新增保持 32pt 输入高度，在尾部加入 24pt“分组与标签”入口；输入 `#` 时在输入框下方 overlay 最多六条 28pt 标签建议，新项明确写“新建标签”，归档或已合并命中显示阻塞原因，只有完整事务成功后才清空输入。右侧任务详情使用可自动换行的两行紧凑编辑器：第一行的当前分组值本身就是唯一 picker，点击该值直接打开菜单，无分组时以“选择分组”占位，旁边只保留语义不同的“管理”入口，不得另放“选择”或“更换”按钮；第二行展示全部标签。当前／未来任务可编辑，历史任务显示带“当时”标记的冻结值、现名关联及独立“追加更正”入口。设置页增加“分组与标签”pane：顶部 30pt 分组／标签／已归档 segmented control、32pt 搜索框、44pt 线性列表行、右侧 280pt inspector；改名和颜色是轻操作，归档、硬删除和合并必须先进入完整影响预览 sheet，sheet 基准约 720×560pt，显示任务数、历史数、去重数、逐项展开和明确确认按钮。所有动效使用 160–220ms ease-in-out，列表、popover 和详情切换保留视觉轨迹；键盘建议支持上下选择、Enter 接受、Esc 关闭；每个移除按钮和 badge 都有目标相关 accessibility label。烛龙批量整理沿用已确认流式会话：左侧烛龙工作、右侧用户决定，before/after 与理由逐项展示，重大批量确认块全宽，确认后在原流中追加 receipt 并平滑滚动。

任务池是上述行内分组 badge 的唯一情境例外：列表已经用分组标题建立归属，行内不再重复同一分组视觉单位，只显示标签；默认窄窗下标签按 `2 → 1 → 0` 个自适应，未显示项统一进入可点击的 `+N`，每个实际显示的 badge 必须保留文字固有宽度。任务池行内不显示排期或删除操作，也不建立第二条操作行；右键菜单提供排期到今天、排期到明天、选日期和删除，并与详情三点菜单共享同一动作定义和执行路径。
...
- 可编辑详情中，当前分组值就是切换 picker；不得用一个被动分组 chip 再叠加独立“选择／更换”按钮制造重复入口。
