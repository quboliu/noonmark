...
- 风险等级：P
- 状态：诊断实现、运行隔离与自动化故障闭环已落地；2026-08-04 同类故障复发的真实诊断证据（`FAIL-2026-08-04-22`）已到达并完成层级归因，合并拒绝 reason 子码诊断增强已实现；私有 DMG `0.2.1 (6)` 已完成受控发行验证，详见 `docs/releases/v0.2.1.md`
...
- 版本与发行身份能力已经落地；`0.2.1 (6)` 包含 0.2.0 (5) 的 MainActor 饥饿根因修复、WindowServer 发行验证回归修复与腾讯输入发行门禁收敛，以及 `FAIL-2026-08-04-22` 复发诊断后落地的 current-record 合并拒绝 reason 子码（250-259）。该 build 已通过真实 `dmg-validation`、完整 `make check` 与 clean-source 重打。关于后续版本的唯一来源、精确 tag、build 递增与兼容审核，统一见 `docs/engineering/versioning-and-private-release.md`。
