...
public extension SuntraceEngine {
...
public extension SuntraceEngine {
    func classification(_ query: ClassificationQuery) throws -> ClassificationProjection {
        ...
        case let .history(traceID):
            ...
            guard let snapshot = classificationState.snapshotsByTraceID[traceID] else {
                throw SuntraceError.notFound("trace classification snapshot")
            }
    ...
    func commitClassification(
        ...
        guard confirmation.authorizes(plan) else {
            throw SuntraceError.invalidInput("classification confirmation does not authorize this plan")
        }
        let decisionID = confirmation.decisionID
        guard try plan.hasValidDigest() else {
            throw SuntraceError.invalidInput("classification plan digest does not match its contents")
        }
        guard plan.blockers.isEmpty else {
            throw SuntraceError.invalidTransition("classification plan has unresolved blockers")
        }
        if let committed = classificationState.committedReceiptsByInteractionID[plan.interactionID] {
            guard committed.planID == plan.id else {
                throw SuntraceError.invalidInput("classification interaction id was already used")
            }
            guard committed.decisionID == decisionID else {
                throw SuntraceError.invalidInput("classification interaction decision does not match")
            }
            guard let record = classificationState.changeRecords.first(where: {
                ...
                throw SuntraceError.invalidInput("classification receipt is missing its matching audit record")
            }
            return committed
        }
        guard plan.baseRevision == classificationState.revision else {
            throw SuntraceError.invalidTransition("classification plan is stale")
        }
    ...
    private func validatePreparedClassificationPlan(
        ...
        guard expectedChanges == plan.changes,
            ...
            throw SuntraceError.invalidInput("classification plan changes do not match its intent")
        }
    }
    ...
    private func validatePlannedCreationIDs(
        ...
        case .createCategory, .createLabel:
            guard plan.createdItemID != nil,
                ...
                throw SuntraceError.invalidInput("classification create plan identities are inconsistent")
            }
        case let .setCurrent(draft):
        ...
        default:
            guard plan.createdItemID == nil,
                ...
                throw SuntraceError.invalidInput("classification plan has unexpected created identities")
            }
        }
    }
    ...
    private func validateCurrentPlanCreationIDs(
        ...
        guard (plan.plannedCreations.categoryID != nil) == createsCategory,
            ...
            throw SuntraceError.invalidInput("classification current plan identities are inconsistent")
        }
        if let categoryRawID = plan.plannedCreations.categoryID?.rawValue {
            guard plannedRawIDs.contains(categoryRawID) == false else {
                throw SuntraceError.invalidInput("classification planned identities are not unique")
            }
        }
    }
    ...
    private func applyClassificationCreate(
        ...
        guard let rawID = plan.createdItemID else {
            throw SuntraceError.invalidInput("classification plan is missing its created identity")
        }
        switch plan.intent {
        case let .createCategory(name, colorHex):
            ...
            guard state.categories[id] == nil, state.categoryDeletionTombstones[id] == nil else {
                throw SuntraceError.invalidInput("task category identity already exists")
            }
        ...
        case let .createLabel(name, colorHex):
            ...
            guard state.labels[id] == nil, state.labelDeletionTombstones[id] == nil else {
                throw SuntraceError.invalidInput("task label identity already exists")
            }
            state.labels[id] = TaskLabel(id: id, name: name, colorHex: colorHex, now: mutation.now)
        default:
            throw SuntraceError.invalidInput("classification plan is not a create intent")
        }
    }
    ...
    private func applyClassificationMerge(
        ...
        default:
            throw SuntraceError.invalidInput("classification plan is not a merge intent")
        }
    }
    ...
    private func applyClassificationHardDelete(
        ...
        default:
            throw SuntraceError.invalidInput("classification plan is not a hard-delete intent")
        }
    }
    ...
    private func applyCurrentClassification(
        ...
        guard case let .setCurrent(draft) = plan.intent else {
            throw SuntraceError.invalidInput("classification plan is not a current classification intent")
        }
        guard chains[draft.chainID] != nil else {
            throw SuntraceError.notFound("task chain")
        }
    ...
    private func applyClassificationRename(
        ...
        default:
            throw SuntraceError.invalidInput("classification plan is not a rename intent")
        }
    }
    ...
    private func applyClassificationLifecycle(
        ...
        default:
            throw SuntraceError.invalidInput("classification plan is not a lifecycle intent")
        }
    }
}

extension SuntraceEngine {
        ...
        let category: ClassificationPlanItem? = try draft.category.map { choice in
            ...
            case let .new(name, colorHex):
                guard let id = plannedCreations.categoryID else {
                    throw SuntraceError.invalidInput("classification plan is missing its planned category identity")
                }
                return ClassificationPlanItem(
                    id: id.description,
                    kind: .category,
        ...
        let labels = try draft.labels.map { choice in
            ...
            case let .new(name, colorHex):
                guard let id = plannedCreations.labelIDsByCanonicalKey[classificationKey(name)] else {
                    throw SuntraceError.invalidInput("classification plan is missing its planned label identity")
                }
                return ClassificationPlanItem(
                    id: id.description,
                    kind: .label,
    ...
    func resolve(
        ...
        case let .existing(id):
            ...
            guard category.lifecycle == .active
                ...
                throw SuntraceError.invalidTransition("task category is not active")
            }
            return id
        case let .new(name, colorHex):
            ...
            guard let plannedID,
                ...
                throw SuntraceError.invalidInput("task category planned identity is unavailable")
            }
    ...
    func resolve(
        ...
        case let .existing(id):
            ...
            guard label.lifecycle == .active
                ...
                throw SuntraceError.invalidTransition("task label is not active")
            }
            return id
        case let .new(name, colorHex):
            let normalizedName = try normalizedClassificationName(name)
            ...
            guard let plannedID,
                ...
                throw SuntraceError.invalidInput("task label planned identity is unavailable")
            }
    ...
    func normalizedClassificationName(_ name: String) throws -> String {
        ...
        guard normalized.isEmpty == false else {
            throw SuntraceError.invalidInput("classification name cannot be empty")
        }
        guard ClassificationNameCanonicalizer.canonicalKey(normalized).isEmpty == false else {
            throw SuntraceError.invalidInput("classification name has no searchable Unicode content")
        }
        return normalized
    }
    ...
    private func preparedCreateIntent(
        ...
        case let .createCategory(name, colorHex):
            ...
            guard classificationState.categories.values.contains(where: {
                ...
                throw SuntraceError.invalidInput("task category name already exists")
            }
        ...
        case let .createLabel(name, colorHex):
            ...
            guard classificationState.labels.values.contains(where: {
                ...
                throw SuntraceError.invalidInput("task label name already exists")
            }
            return (.createLabel(name: displayName, colorHex: colorHex), [])
        default:
            throw SuntraceError.invalidInput("classification intent is not a create intent")
        }
    }
    ...
    private func preparedMergeIntent(
        ...
        default:
            throw SuntraceError.invalidInput("classification intent is not a merge intent")
        }
    }

    private func validateCategoryMerge(source: TaskCategoryID, target: TaskCategoryID) throws {
        guard source != target else {
            throw SuntraceError.invalidInput("task category cannot be merged into itself")
        }
        guard let sourceCategory = classificationState.categories[source],
              let targetCategory = classificationState.categories[target]
        else {
        ...
        guard sourceCategory.lifecycle == .active || sourceCategory.lifecycle == .archived,
            ...
            throw SuntraceError.invalidTransition("task category already has a merge target")
        }
        guard targetCategory.lifecycle == .active else {
            throw SuntraceError.invalidTransition("merge target task category is not active")
        }
    }

    private func validateLabelMerge(source: TaskLabelID, target: TaskLabelID) throws {
        guard source != target else {
            throw SuntraceError.invalidInput("task label cannot be merged into itself")
        }
        guard let sourceLabel = classificationState.labels[source],
              let targetLabel = classificationState.labels[target]
        else {
        ...
        guard sourceLabel.lifecycle == .active || sourceLabel.lifecycle == .archived,
            ...
            throw SuntraceError.invalidTransition("task label already has a merge target")
        }
        guard targetLabel.lifecycle == .active else {
            throw SuntraceError.invalidTransition("merge target task label is not active")
        }
    }
    ...
    private func preparedHardDeleteIntent(
        ...
        default:
            throw SuntraceError.invalidInput("classification intent is not a hard-delete intent")
        }
    }
    ...
    private func validateHardDeleteCategory(_ id: TaskCategoryID) throws {
        ...
        guard category.lifecycle == .active || category.lifecycle == .archived else {
            throw SuntraceError.invalidTransition("merged task category cannot be hard deleted")
        }
    }
    ...
    private func validateHardDeleteLabel(_ id: TaskLabelID) throws {
        ...
        guard label.lifecycle == .active || label.lifecycle == .archived else {
            throw SuntraceError.invalidTransition("merged task label cannot be hard deleted")
        }
    }
    ...
    private func preparedRenameIntent(
        ...
        default:
            throw SuntraceError.invalidInput("classification intent is not a rename intent")
        }
    }
    ...
    private func preparedCategoryRename(id: TaskCategoryID, name: String) throws -> ClassificationIntent {
        ...
        guard category.lifecycle != .merged else {
            throw SuntraceError.invalidTransition("merged task category cannot be renamed")
        }
        guard classificationState.categories.values.contains(where: {
    ...
    private func preparedLabelRename(id: TaskLabelID, name: String) throws -> ClassificationIntent {
        ...
        guard label.lifecycle != .merged else {
            throw SuntraceError.invalidTransition("merged task label cannot be renamed")
        }
        guard classificationState.labels.values.contains(where: {
            $0.id != id && $0.allCanonicalKeys.contains(key)
        }) == false else {
    ...
    private func preparedLifecycleIntent(
        ...
        case let .archiveCategory(id):
            ...
            guard classificationState.categoryMerges.values.contains(where: { $0.targetID == id }) == false else {
                throw SuntraceError.invalidTransition("merge target task category must remain active")
            }
        case let .restoreCategory(id):
        ...
        case let .archiveLabel(id):
            ...
            guard classificationState.labelMerges.values.contains(where: { $0.targetID == id }) == false else {
                throw SuntraceError.invalidTransition("merge target task label must remain active")
            }
        case let .restoreLabel(id):
            try requireLabelLifecycle(id, is: .archived)
        default:
            throw SuntraceError.invalidInput("classification intent is not a lifecycle intent")
        }
        return (intent, [])
    }
    ...
    func renameCategory(
        ...
        guard now >= category.updatedAt,
            ...
            throw SuntraceError.invalidTransition("task category rename time moved backwards")
        }
    ...
    func requireCategoryLifecycle(
        ...
        guard category.lifecycle == lifecycle else {
            throw SuntraceError.invalidTransition("task category lifecycle is not \(lifecycle.rawValue)")
        }
    }
    ...
    func transitionCategory(
        ...
        guard category.lifecycle == expected else {
            throw SuntraceError.invalidTransition("task category lifecycle changed")
        }
        guard now >= category.updatedAt else {
            throw SuntraceError.invalidTransition("task category lifecycle time moved backwards")
        }
    ...
    func renameLabel(
        ...
        guard now >= label.updatedAt,
            ...
            throw SuntraceError.invalidTransition("task label rename time moved backwards")
        }
    ...
    func requireLabelLifecycle(
        ...
        guard label.lifecycle == lifecycle else {
            throw SuntraceError.invalidTransition("task label lifecycle is not \(lifecycle.rawValue)")
        }
    }
    ...
    func transitionLabel(
        ...
        guard label.lifecycle == expected else {
            throw SuntraceError.invalidTransition("task label lifecycle changed")
        }
        guard now >= label.updatedAt else {
            throw SuntraceError.invalidTransition("task label lifecycle time moved backwards")
        }
    ...
    func hardDeleteCategory(
        ...
        guard categoryReferenceSummary(id, in: state).isEmpty else {
            throw SuntraceError.invalidTransition("used task category cannot be hard deleted")
        }
        guard mutation.now >= category.updatedAt else {
            throw SuntraceError.invalidTransition("task category deletion time moved backwards")
        }
    ...
    func hardDeleteLabel(
        ...
        guard labelReferenceSummary(id, in: state).isEmpty else {
            throw SuntraceError.invalidTransition("used task label cannot be hard deleted")
        }
        guard mutation.now >= label.updatedAt else {
            throw SuntraceError.invalidTransition("task label deletion time moved backwards")
        }
        state.labels[id] = nil
    ...
    func mergeCategory(
        ...
        guard sourceID != targetID,
            ...
            throw SuntraceError.invalidInput("invalid task category merge")
        }
        ...
        guard mutation.now >= source.updatedAt, mutation.now >= target.updatedAt else {
            throw SuntraceError.invalidTransition("task category merge time moved backwards")
        }
    ...
    func mergeLabel(
        ...
        guard sourceID != targetID,
            ...
            throw SuntraceError.invalidInput("invalid task label merge")
        }
        guard source.lifecycle == .active || source.lifecycle == .archived,
        ...
        guard mutation.now >= source.updatedAt, mutation.now >= target.updatedAt else {
            throw SuntraceError.invalidTransition("task label merge time moved backwards")
        }
    ...
    func validateCurrentRelationMutationTime(
        ...
        guard latestRelationTime.map({ mutationTime >= $0 }) ?? true else {
            throw SuntraceError.invalidTransition("task classification relation time moved backwards")
        }
    }
    ...
    func inheritCurrentClassification(
        ...
        guard now.timeIntervalSinceReferenceDate.isFinite else {
            throw SuntraceError.invalidInput("classification audit time must be finite")
        }
