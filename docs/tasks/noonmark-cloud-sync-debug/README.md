...
- 版本与发行身份能力已经落地；当前候选预留为 `0.2.0 (5)`，包含异步 App 入口造成的 MainActor 饥饿根因修复、WindowServer 发行验证回归修复与腾讯输入发行门禁收敛。build 5 在真实 `dmg-validation`、完整 `make check` 与最终 clean-source 重打全部通过前不得宣称已交付。“关于晷迹”与发行 manifest 可对账 Version、Build、Commit、UTC Build Date、OS 与架构，打包链另外保留 Mach-O UUID、binary SHA、dSYM 和 DMG identity。
