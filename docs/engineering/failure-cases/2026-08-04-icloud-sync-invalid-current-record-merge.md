...
- 实际修改者：未知
- 修复提交：699901d0ac23eac3b3930084e3d64c6582495dca（根本修复回填，详见 FAIL-2026-08-06-01）
...
根本修复（2026-08-06 回填）：2026-08-06 同类事故第二次复发（案例 `FAIL-2026-08-06-01`），250-259 子码按本案例预设的归因路径直接读出被违反的规则为 251（内容／突变时钟）。审计与复现定位到 `mergeSubtask` 把合法可变字段（`title/position/traceID/carriedFromSubtaskID`）误作不可变身份（引入提交 `cd7f29f935e0d931139d6df9d130530b7537976d`），修复将身份 guard 收窄为 `lineageID + createdAt`、可变字段恢复 LWW 内容语义，复现测试由红转绿。本事故（08-04）的毒记录实体身份与规则不可考（当时无子码），但审计证明 250-259 各合并时规则点中只有该 guard 存在真实写入可达路径，同类机制已修复并带 fast + symptom 永久门禁；修复 commit 与证据见 `2026-08-06-subtask-mutable-identity-merge.md`。
